...
if False:
    ...
    os.chdir('/home/jaesungyoo/FL/train')
    # args.exp_dir = 'multirun/Distil/diabetes/snuh/CDS/2021-06-27/00-40-06'
    args.exp_dir = 'exp/Distil_paper/thyroid/HP_search'
    args.result = 'result/server_result_global_list.p'

    os.listdir(os.path.join(args.exp_dir, '0'))
    os.getcwd()
...
#     args.result = 'result/result_global_list.p'
#     result_list = []
#     walk(args.exp_dir, exp_dict={'exp':os.path.basename(args.exp_dir)})
#     result_table = pd.DataFrame(result_list)
#     # result_table.to_csv(os.path.join(args.exp_dir, f'result_global_list.csv'), index=False)
#     result_table.to_excel(os.path.join(args.exp_dir, f'result_global_list.xlsx'), index=False)
#
#     args.result = 'result/result_local_list.p'
#     result_list = []
#     walk(args.exp_dir, exp_dict={'exp':os.path.basename(args.exp_dir)})
#     result_table = pd.DataFrame(result_list)
#     # result_table.to_csv(os.path.join(args.exp_dir, f'result_local_list.csv'), index=False)
#     result_table.to_excel(os.path.join(args.exp_dir, f'result_local_list.xlsx'), index=False)
# else:
...
walk(args.exp_dir, exp_dict={'exp':os.path.basename(args.exp_dir)})
result_table = pd.DataFrame(result_list)
# result_table.to_csv(os.path.join(args.exp_dir, f'{args.savename}.csv'), index=False)
